const API_BASE_URL = "https://api.coingecko.com/api/v3";
        ...
        async function fetchCryptoData() {
            try {
                ...
                document.getElementById("cryptoListContainer").innerHTML = `<p>Error loading data. <button onclick="fetchCryptoData()" class="btn">Retry</button></p>`;
                console.error(error);
            }
        }
        ...
        function renderCryptoList() {
            ...
            container.innerHTML = sortedCryptoData.map((crypto) => `
                <div class="crypto-item ${comparedCryptos.includes(crypto.id) ? 'selected' : ''}">
                    <div>
                        <img src="${crypto.image}" alt="${crypto.name}">
                        <strong>${crypto.name}</strong> <small>(${crypto.symbol.toUpperCase()})</small>
                    </div>
                    <div>
                        <div>${formatCurrency(crypto.current_price)}</div>
                        <div class="${crypto.price_change_percentage_24h >= 0 ? 'price-up' : 'price-down'}">
                    ...
                    <button class="btn" aria-label="Compare ${crypto.name}" 
                        ...
                        ${comparedCryptos.includes(crypto.id) ? 'Added' : 'Compare'}
                    </button>
                </div>
            `).join("");
        }
        ...
        function renderComparisonSection() {
            ...
            container.innerHTML = comparedCryptos.map((id) => {
                const crypto = cryptoData.find((c) => c.id === id);
                return `
                    <div class="comparison-card">
                        <h3>${crypto.name} (${crypto.symbol.toUpperCase()})</h3>
                        <img src="${crypto.image}" alt="${crypto.name}" style="width:50px; margin-bottom:10px;">
                        <p>Price: ${formatCurrency(crypto.current_price)}</p>
                        <p class="${crypto.price_change_percentage_24h >= 0 ? 'price-up' : 'price-down'}">
                            ${crypto.price_change_percentage_24h > 0 ? '+' : ''}${crypto.price_change_percentage_24h.toFixed(2)}%
                        </p>
                        <button class="btn" onclick="toggleComparison('${crypto.id}')">Remove</button>
                    </div>
                `;
            }).join("");
        }
        ...
        function sortCryptoList() {
            ...
            sortedCryptoData.sort((a, b) => {
                ...
                if (criteria === "change") return b.price_change_percentage_24h - a.price_change_percentage_24h;
            });
            renderCryptoList();
        }
